  ...
  public class DemoStream {
    ...
    public static void map03() {
      ...
      manNames.stream().map(String::toUpperCase).map(String::toLowerCase).forEach(System.out::println);
    }
    ...
    public static void reduce01() {
      ...
      Integer result = manNames.stream().reduce(1, (a, b) -> a * b); // gập các phần tử lại để tính tổng
    ...
    public static void agefilter() {
      people.stream().filter(p -> (p.getAge() >= 20 && p.getAge() <= 30)).forEach(System.out::println);
    }

    // Tuổi trung bình
    public static void aveAge() {
      System.out.println(people.stream().mapToInt(p -> p.getAge()).average());
    }

    // Tuổi trung bình theo từng quốc tịch
    public static void avgOfCountry() {
